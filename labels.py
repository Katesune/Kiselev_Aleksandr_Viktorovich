# -*- coding: utf-8 -*-
"""
Created on Sun Nov  8 22:09:23 2020
...
#Моя функция
def myFunction(labels):
    label = 1
    label_it = {}
    for row in range(labels.shape[0]):
        for col in range(labels.shape[1]):
            if labels[row, col]!=0:
                if label_it.get(labels[row,col])==None:
                    label_it[labels[row, col]] = label
                    label+=1
   
    for row in range(labels.shape[0]):
        for col in range(labels.shape[1]):
            if labels[row, col]!=0:
                labels[row, col] = label_it.get(labels[row,col])
